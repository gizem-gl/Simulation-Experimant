...
# ╔═╡ 37eefbeb-fce0-4337-81c6-c369bb5a5a21
rs_euler = euler(eom, r0, par, tmax, dt)

# ╔═╡ 9da0021f-e5e5-40a4-977c-6a7e2c6e5dce
rs_pred = predictor_corrector(eom, r0, par, tmax, dt)

# ╔═╡ 6d29985d-abaf-4f9d-b3d0-7a383df09e16
...
# ╔═╡ 8915f2ec-4207-4487-83da-d9d540f9c109
...
plot([r[1] for r in rs_euler],[r[2] for r in rs_euler], xlabel="t", ylabel="x")

# ╔═╡ 3155e231-55c9-4860-8b96-286ac42f2b34
plot([r[1] for r in rs_euler],[r[3] for r in rs_euler], xlabel="t", ylabel="x")

# ╔═╡ 4861899c-c09b-4559-a71a-a63f96cbb0f0
plot([r[1] for r in rs_euler],[r[4] for r in rs_euler], xlabel="t", ylabel="x")

# ╔═╡ b069e394-326f-4477-9d8d-269dcd220c0a
plot3d([r[4] for r in rs_euler],[r[2] for r in rs_euler],[r[3] for r in rs_euler])

# ╔═╡ c954805d-ade2-421b-99e6-a87288870eb1
plot3d([r[1] for r in rs_euler],[r[2] for r in rs_euler],[r[3] for r in rs_euler])

# ╔═╡ e2994ebb-448a-445c-90e6-90bd053a085b
plot([r[1] for r in rs_pred],[r[2] for r in rs_pred], xlabel="t", ylabel="x")

# ╔═╡ 8b24b2a5-ad7c-4457-8f48-ba35dea6b12a
plot([r[1] for r in rs_pred],[r[3] for r in rs_pred], xlabel="t", ylabel="x")

# ╔═╡ 817b30ee-f9a4-4e4d-9860-dfb6294831d9
plot([r[1] for r in rs_pred],[r[4] for r in rs_pred], xlabel="t", ylabel="x")

# ╔═╡ 5e2ff0ef-6b15-436b-828f-973f915bc4c9
plot3d([r[4] for r in rs_pred],[r[2] for r in rs_pred],[r[3] for r in rs_pred])

# ╔═╡ 1a09c1a3-934a-44c7-90f4-ffdb1c6dc8e9
plot3d([r[1] for r in rs_pred],[r[2] for r in rs_pred],[r[3] for r in rs_pred])

# ╔═╡ f93e71bf-6d22-4186-bf06-746c6ac31a06
plot([r[1] for r in rs_rk4],[r[2] for r in rs_rk4], xlabel="t", ylabel="x")

# ╔═╡ 6315d0f4-4c72-45c4-b716-466039e18b62
plot([r[1] for r in rs_rk4],[r[3] for r in rs_rk4], xlabel="t", ylabel="x")

# ╔═╡ f8348727-4d08-4c5c-b603-9fe227e9e3e6
plot([r[1] for r in rs_rk4],[r[4] for r in rs_rk4], xlabel="t", ylabel="x")

# ╔═╡ 67560461-8da4-4cd6-ad86-b312cc6bfd5f
plot3d([r[4] for r in rs_rk4],[r[2] for r in rs_rk4],[r[3] for r in rs_rk4])

# ╔═╡ d0694c06-bc7d-4fa4-bab9-721f0ce0af83
plot3d([r[1] for r in rs_rk4],[r[2] for r in rs_rk4],[r[3] for r in rs_rk4])

# ╔═╡ a42b1b10-8936-4f2c-9ac6-a59976d9e6a9
plot([r[1] for r in rs_rk4],[r[5] for r in rs_euler],  xlabel="Time (ns)", ylabel="Energy (GeV)", title="Energy Conservation")

# ╔═╡ 02150194-30c5-4ed6-bb14-11b1b94552c5
plot([r[1] for r in rs_rk4],[r[4] for r in rs_pred],  xlabel="Time (ns)", ylabel="Energy (GeV)", title="Energy Conservation")

# ╔═╡ 1f28eaba-b0b4-4be0-953f-c1697d77a88b
plot([r[1] for r in rs_rk4],[r[4] for r in rs_rk4], xlabel="Time (ns)", ylabel="Energy (GeV)", title="Energy Conservation")
...
function hit_and_miss_sampling(bin_contents, bin_positions, num_samples)	
	...
	for i in 1:num_samples
		...
		if random_y <= bin_contents[random_x]
            accepted_samples_hm[random_x] += 1  # Increment bin count
        end
	end
...
begin
	...
	histogram(bin_positions, weights=accepted_samples_hm, bins=62, xlabel="Mass (MeV/c²)", ylabel="Candidates", title="Hit and Miss Simulated Sample", legend=false)
end
...
begin
	...
	### Plot histogram with the simulated sample
	histogram(bin_positions, weights=accepted_samples_inv, bins=62, xlabel="Mass (MeV/c²)", ylabel="Candidates", title="Inverse CDF Simulated Sample", legend=false)
end
...
# ╔═╡ 0d2d8a57-3823-429d-aefc-2de22166fb4d
### Compare performances of the two methods
println("Chi² Hit and Miss: $chi2_hm")

# ╔═╡ 801b3f96-a101-43ba-a69b-33afc0c471fd
println("Chi² Inverse CDF: $chi2_inv")

# ╔═╡ 35d9573e-87d1-4e2d-9ed1-93f8b18e90ab
println("Efficiency Hit and Miss: $eff_hm")

# ╔═╡ 012d3555-e2fc-48e8-b682-990cce0387c7
println("Efficiency Inverse CDF: $eff_inv")

# ╔═╡ a9736215-0333-4c63-ada0-42c514916289
println("Inverse CDF method is $(eff_inv/eff_hm) times more effective than hit and miss method.")
...
begin
	...
	### Simulate invariant mass of the J/psi using function from the Exersice 2
	M_mumu = rand(Uniform(3.0, 3.2), Nevents).*1.0u"GeV/c^2" 
	M_mumu_sim = []
	pmu_minus = []
	pmu_plus = []
	pmu_minus_sim = []
	...
	for M in M_mumu
		### Using functions provided in this exersice, implement simulation of the J/psi decay into two muons, simulate trajectory of the muons with Runge-Kutta method and reconstruct muons momenta. Fill all the arrays, initialized above. Compare reconstructed momenta with initial.
				# Simulate J/psi decay into two muons
		...
		p1_sim = [rs_minus[end][5], rs_minus[end][6], rs_minus[end][7], rs_minus[end][8]]
        p2_sim = [rs_plus[end][5], rs_plus[end][6], rs_plus[end][7], rs_plus[end][8]]
		
        # Save the simulated muon momenta
        push!(pmu_minus, p1)
        push!(pmu_plus, p2)
		
	    push!(pmu_minus_sim, p1_sim)
        push!(pmu_plus_sim, p2_sim)
		
        # Simulate trajectory using numerical integration (placeholder)
        push!(pmu_minus_sim, p1)  # Simplified assumption
        push!(pmu_plus_sim, p2)
		
        # Reconstruct the invariant mass of the J/psi
        M_reconstructed = jpsi_from_mumu(p1_sim, p2_sim)
        push!(M_mumu_sim, M_reconstructed)
	end
end

# ╔═╡ 53537b9f-226b-4ea1-8aff-3d5ed4034e6f
# Loop through all events to compare reconstructed momenta with initial
for i in 1:Nevents
    println("Event $i:")

    # Initial and simulated momenta for minus muon
    initial_minus = pmu_minus[i]
    simulated_minus = pmu_minus_sim[i]

    # Initial and simulated momenta for plus muon
    initial_plus = pmu_plus[i]
    simulated_plus = pmu_plus_sim[i]

    # Differences for minus muon
    Δpx_minus = abs(simulated_minus[2] - initial_minus[2])
    Δpy_minus = abs(simulated_minus[3] - initial_minus[3])
    Δpz_minus = abs(simulated_minus[4] - initial_minus[4])
    Δp_mag_minus = sqrt(Δpx_minus^2 + Δpy_minus^2 + Δpz_minus^2)

    # Differences for plus muon
    Δpx_plus = abs(simulated_plus[2] - initial_plus[2])
    Δpy_plus = abs(simulated_plus[3] - initial_plus[3])
    Δpz_plus = abs(simulated_plus[4] - initial_plus[4])
    Δp_mag_plus = sqrt(Δpx_plus^2 + Δpy_plus^2 + Δpz_plus^2)

    # Print differences for minus muon
    println("  Minus Muon:")
    println("    Δpx: $Δpx_minus")
    println("    Δpy: $Δpy_minus")
    println("    Δpz: $Δpz_minus")
    println("    Δ|p|: $Δp_mag_minus")

    # Print differences for plus muon
    println("  Plus Muon:")
    println("    Δpx: $Δpx_plus")
    println("    Δpy: $Δpy_plus")
    println("    Δpz: $Δpz_plus")
    println("    Δ|p|: $Δp_mag_plus")

    # Reconstructed and initial invariant mass comparison
    println("  Invariant Mass Comparison:")
    println("    Initial Mass: $(M_mumu[i])")
    println("    Reconstructed Mass: $(M_mumu_sim[i])")
